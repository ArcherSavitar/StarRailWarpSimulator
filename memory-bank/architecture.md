...
### 10.1 布局组件 (src/components/Layout.tsx)
- 主布局容器，深色主题背景 (#0D1117)
- Header: 标题和副标题
- Main: 内容区域
- Footer: 版权信息

### 10.2 卡池选择器 (src/components/BannerSelector.tsx)
- 显示所有可用卡池
- 点击切换当前卡池
- 高亮当前选中的卡池
- 显示UP角色/武器信息

### 10.3 保底计数器 (src/components/PityCounter.tsx)
- 显示5星保底进度条 (0-90/80)
- 显示4星保底进度条 (0-10)
- 高亮接近保底时的进度 (70+ 抽橙红色，8+ 抽紫色)
- 大保底状态提示

### 10.4 抽卡按钮 (src/components/PullButtons.tsx)
- 单抽按钮 (紫色)
- 十连按钮 (金色)
- loading 状态防止重复点击

### 10.5 抽卡结果弹窗 (src/components/ResultModal.tsx)
- 弹窗显示抽卡结果
- 按星级排序 (5星 → 4星 → 3星)
- 不同星级不同边框颜色
- "再来十连"和"关闭"按钮

### 10.6 统计面板 (src/components/StatsPanel.tsx)
- 总抽数、5星数、4星数
- 总体5星/4星概率
- 当前卡池抽数统计
- 当前卡池5星概率
